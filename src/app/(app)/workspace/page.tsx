...
import { buildDifficultyChallenge, type ResearchDifficulty } from "@/lib/topics";
    ...
    return (
      <main className="mx-auto flex min-h-screen max-w-2xl items-center px-6">
        <Card className="w-full">
          ...
          <CardContent>
            <p className="text-sm text-destructive">Refresh the page or try again shortly.</p>
          </CardContent>
        </Card>
      </main>
    );
      ...
      <main className="mx-auto flex min-h-screen max-w-3xl items-center px-6 py-12">
        <Card className="w-full">
          <CardHeader>
            ...
            <CardTitle className="mt-3 text-3xl">Choose what Curio should uncover next.</CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            <p className="text-muted-foreground">
              Pick a field, spin for a topic, then choose how long you want to focus.
            </p>
            {pageError ? <p className="text-sm text-destructive">{pageError}</p> : null}
            <Link className="inline-block font-medium text-primary" href="/onboarding">
              Open the topic wheel
            </Link>
            <Link className="block text-sm font-medium text-muted-foreground" href="/dashboard">
              Back to dashboard
            </Link>
          </CardContent>
        </Card>
  ...
  return (
    <ResearchWorkspaceShell
      ...
      topic={{
        ...
        difficulty: (session.difficulty_level ?? "beginner") as ResearchDifficulty,
